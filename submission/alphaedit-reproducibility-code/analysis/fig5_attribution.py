...
Panels:
  A. Cumulative performance at 10K (grouped dot plot: MEMIT vs AlphaEdit vs SeqReg)
  B. Retention/plasticity decomposition at 10K
  C. Functional projection preservation (survival fraction by method × ordering)
...
def panel_a_cumulative_performance(ax):
    """Panel A: Cumulative performance at 10K (grouped dot plot)."""
    seed = 42
    ...
    if not methods:
        ...
        ax.set_title("(a) Cumulative Performance at 10K")
        return
    ...
    ax.set_ylabel("Score")
    ax.set_title("(a) Cumulative Performance at 10K Edits")
    ax.legend(loc="upper right", fontsize=8)
    ax.set_ylim(0, 1.1)
    ax.axhline(0.5, color="gray", linestyle=":", alpha=0.3)


def panel_b_retention_decomposition(ax):
    """Panel B: Retention/plasticity decomposition at 10K."""
    seed = 42
    ordering = "key_clustered"
    ckpt = "10000_edits"
    ...
    if not methods:
        ax.text(0.5, 0.5, "No data available", transform=ax.transAxes,
                ha="center", va="center", fontsize=11)
        ax.set_title("(b) Retention/Plasticity Decomposition")
        return
    ...
    ax.set_ylabel("Score")
    ax.set_title("(b) Retention/Plasticity at 10K (seed 42)")
    ax.legend(loc="lower right", fontsize=8)
    ax.set_ylim(0.5, 1.05)


def panel_c_functional_preservation(ax, mechanism: dict):
    """Panel C: Functional projection preservation by method × ordering."""
    layer_data = mechanism.get("joint_model_layer6", {})
    if not layer_data:
        ax.text(0.5, 0.5, "No mechanism summary data", transform=ax.transAxes,
                ha="center", va="center", fontsize=11)
        ax.set_title("(c) Functional Preservation")
        return
    ...
    ax.set_title("(c) Functional Preservation by Method \u00d7 Ordering")
...
def generate(output_dir: Path = PAPER_OUTPUT):
    ...
    panel_a_cumulative_performance(axes[0])
    panel_b_retention_decomposition(axes[1])
    panel_c_functional_preservation(axes[2], mechanism)
