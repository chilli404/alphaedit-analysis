...
Panels (Panel A is a manual schematic — skipped here):
  B. Retention outcome under controlled concentration (paired points)
  C. Geometric response (within-batch cosine by ordering)
  D. Age-matched retained vs forgotten future-key cosine
  E. Effect summary (forest plot: OR per +0.1 cosine with 95% CI)
...
def panel_b_retention_under_concentration(ax):
    ...
    ax.set_ylabel("Efficacy at 5K")
    ax.set_title("(B) Retention Under Controlled Concentration")
    ax.legend(loc="lower left", fontsize=7, ncol=2)
    ax.set_ylim(0.4, 1.05)
    ax.axhline(0.5, color="gray", linestyle=":", alpha=0.3)


def panel_c_geometric_response(ax):
    """Panel C: Within-batch cosine similarity by ordering."""
    bar_data = []
    ...
    if not bar_data:
        ...
        ax.set_title("(C) Geometric Response")
        return
    ...
    ax.set_title("(C) Stream Geometric Concentration")
...
def panel_d_age_matched_cosine(ax, results: dict):
    """Panel D: Age-matched retained vs forgotten future-key cosine."""
    ks = results.get("age_matched", {}).get("key_similarity", {})
    if not ks:
        ax.text(0.5, 0.5, "No key similarity data", transform=ax.transAxes,
                ha="center", va="center", fontsize=11)
        ax.set_title("(D) Key Cosine: Survived vs Forgotten")
        return
    ...
    ax.set_title("(D) Key Geometry Predicts Forgetting")
    ax.legend(loc="upper left", fontsize=8)
    ax.set_ylim(0, max(forgotten_vals) * 1.15 if forgotten_vals else 1.0)


def panel_e_forest_plot(ax, results: dict):
    """Panel E: Forest plot — OR per +0.1 future-key cosine with 95% CI."""
    bootstrap = results.get("bootstrap", {})
    if not bootstrap:
        ax.text(0.5, 0.5, "No bootstrap data", transform=ax.transAxes,
                ha="center", va="center", fontsize=11)
        ax.set_title("(E) Effect Size: OR per +0.1 Cosine")
        return
    ...
    ax.set_title("(E) Effect Size (Block Bootstrap 95% CI)")
...
def generate(output_dir: Path = PAPER_OUTPUT):
    ...
    panel_b_retention_under_concentration(axes[0, 0])
    panel_c_geometric_response(axes[0, 1])
    panel_d_age_matched_cosine(axes[1, 0], results)
    panel_e_forest_plot(axes[1, 1], results)
