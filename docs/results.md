...
### Conditional Computations and Prerequisites

Some pipeline outputs require prerequisite data that must be generated first. Here is the status:

| Component | Prerequisite | Status | How to Generate |
|-----------|-------------|--------|-----------------|
| **Fig 1, Fig 2, Tables 1/3** | Failure curve per-case JSONs | **PRESENT** | `bash scripts/run_failure_curve_checkpointed.sh <seed> <alg> <edits>` |
| **Fig 5 (interference)** | Key vectors + interference panel | **PRESENT** | `uv run python -m src.mechanism.compute_keys --seed 42 --layer 6` |
| **Table 5 (interference stats)** | `interference_panel_results.json` | **PRESENT** | `make -C analysis interference` |
| **Appendix A8 (SeqReg mechanism)** | MEMIT-Seq JSONL logs (lp1.0-ld1.0) | **PARTIAL** | `bash scripts/run_memit_sequential.sh 42 1 1` |
| **Appendix A9 (polykernel)** | Polykernel per-case results | **PRESENT** | Already computed for seed42, 2K-6K + 10K edits |
| **Appendix capability probe** | Capability probe JSONL per algorithm | **PRESENT** | `bash scripts/run_capability_probe_offline.sh <seed> all` |
| **Method comparison summary** | Failure curve + polykernel + MEMIT-Seq results | **PRESENT** | Auto-generated by `analysis/plots/method_comparison.py` |

**Prerequisite data status (verified):**

- **Key vectors**: `results/key_vectors/seed42/keys_seed42.npz` (105MB, 3000 cases, layer 6) and `seed2024` — COMPLETE
- **Interference panel**: `results/figures/paper/interference_panel_results.json` (18.5KB, 18K observations, full M1-M4 models + negative controls) — COMPLETE
- **Capability probe**: 3 seeds × AlphaEdit/MEMIT (10K edits) + seed42 × MEMIT-Seq (6K edits) — COMPLETE
- **SeqReg logs**: Embedded in matched ordering + failure curve results; standalone mechanism JSONL may need regeneration for A8 with λ_prev=1, λ_delta=1

**To reproduce all findings from scratch:**
```bash
# 1. Core experiments (GPU, run first)
bash scripts/run_failure_curve_checkpointed.sh 42 both 10000
bash scripts/run_memit_sequential.sh 42 1 0        # MEMIT-Seq lp1.0-ld0.0
...
# 2. Prerequisite computations (GPU, run after experiments)
uv run python -m src.mechanism.compute_keys --seed 42 --layer 6
bash scripts/run_capability_probe_offline.sh 42 all

# 3. Analysis pipeline (CPU only, reads results/)
make -C analysis all
make -C analysis appendix
```
...
**Purpose**: Track general model capabilities (WikiText-103 perplexity + few-shot MMLU) during editing to detect when editing damages core language understanding beyond just factual knowledge.

**Configuration**:
- Model: Meta-Llama-3-8B-Instruct
- Metrics: WikiText-103 corpus-level perplexity (200 passages, 512-token max length), 4-subject MMLU (5-shot, 50 per subject)
...
- Edit counts: 3000, 5000, 7000 (seed42 has all three; seeds 137, 2024 have 7000 only)
- Seeds: 42, 137, 2024
- Algorithms: AlphaEdit (complete), MEMIT (partial — seed42/3000edits/order0-1 only)
