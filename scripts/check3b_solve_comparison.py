...
# Load z-targets from cache (generated by previous real runs)
z_layer = hparams.v_loss_layer
print(f"\n  Loading cached z-targets (v_loss_layer={z_layer})...")
cache_dir = Path("share/projects/rewriting-knowledge/kvs/Qwen_Qwen2.5-7B-Instruct_MEMIT")
z_list = []
loaded_from_cache = 0
for i, request in enumerate(requests):
    # Cache filename pattern: mcf_layer_{z_layer}_clamp_{clamp}_case_{case_id}.npz
    cache_fname = cache_dir / f"mcf_layer_{z_layer}_clamp_{hparams.clamp_norm_factor}_case_{request['case_id']}.npz"
    if cache_fname.exists():
        data = np.load(cache_fname)
        z_list.append(torch.from_numpy(data["v_star"]).to("cuda"))
        loaded_from_cache += 1
    else:
        # Fall back to computing (slow, may not converge well in bf16)
        cur_z = compute_z(model, tok, request, hparams, z_layer, context_templates)
        z_list.append(cur_z)
    if (i+1) % 20 == 0:
        print(f"    {i+1}/100 z-targets loaded")
zs = torch.stack(z_list, dim=1)
print(f"  zs shape: {zs.shape}")
print(f"  Loaded {loaded_from_cache}/100 from cache")
