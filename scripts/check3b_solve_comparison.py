...
# NOTE: We skip compute_z entirely.
# The Check 3b question is purely about linear algebra equivalence:
#   solve(LHS, K@R^T)^T  ==  R @ solve(LHS, K)^T   when LHS is symmetric?
# This holds for ANY non-trivial K and R. We use real model keys (realistic)
# and random residuals scaled to match typical z-error magnitude (~100-130).

print()
print("=" * 72)
print("  PER-LAYER COMPARISON (batch 1, 100 edits)")
print("  Using real model keys + random residuals (scaled to ||resid||≈100)")
print("=" * 72)

L2 = 1  # AlphaEdit Qwen L2
alpha = hparams.mom2_update_weight  # 15000
d_out = model.config.hidden_size  # 3584 for Qwen2.5-7B
B = 100  # batch size
...
for i, layer in enumerate(hparams.layers):
    ...
    # Compute REAL keys from the model (these reflect actual activation scale)
    layer_ks = compute_ks(model, tok, requests, hparams, layer, context_templates).T
    # layer_ks shape: (d_in, B) where d_in = intermediate_size for down_proj

    # Generate random residual scaled to realistic magnitude
    # In real runs, z_error ≈ 100-130, spread across 5 layers → resid ≈ 20-25 per layer
    d_in = layer_ks.shape[0]
    n_layers_remaining = len(hparams.layers) - i
    resid = torch.randn(d_out, B, device="cuda") * (100.0 / n_layers_remaining / np.sqrt(d_out))
    # Scale so ||resid||_F ≈ 20-30 (matching real observations)
    resid = resid * (25.0 / torch.linalg.norm(resid).item()) * np.sqrt(B)
